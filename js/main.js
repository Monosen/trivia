...
const fetchDataAPI = e => {
    ...
    let API = `https://opentdb.com/api.php?amount=${number}`;
...
const addQuestion = (question) => {
    boxQuestion.innerHTML = `<p class="text-center fs-4 text-white">${contador+1}. ${question}</p> `;
}
...
const showQuestions = () => {
    ...
    if (answerLenght.length > 1) {
        ...
        orderAnswer.sort((a, b) => {return Math.random() - Math.random()});
        boxAnswer.innerHTML = `<button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-primary mx-2">${orderAnswer[0]}</button>
        <button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-secondary mx-2">${orderAnswer[1]}</button>
        <button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-success mx-2">${orderAnswer[2]}</button>
        <button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-danger mx-2">${orderAnswer[3]}</button>`;
    } else {
        let orderAnswerTwo = [];
        orderAnswerTwo.push(correctAnswer[contador]);
        orderAnswerTwo.push(incorrectAnswer[contador][0]);
        orderAnswerTwo.sort((a, b) => {return Math.random() - Math.random()});
        boxAnswer.innerHTML = `<button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-primary mx-2">${orderAnswerTwo[0]}</button>
        <button onClick="handleCheckAnswer(this)" type="button" class="btn btn-outline-secondary mx-2">${orderAnswerTwo[1]}</button>`;
    }
